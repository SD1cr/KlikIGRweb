...
//    }
    ...
    public function getLoginAjax(Request $Request){
        if ($Request->ajax()) {
            ...
            $typeid = User::Where('email', $email)->pluck('type_id');
            $flagverif = User::Where('email',$email)->where('flag_verif', 1)->count();

//            dd($flagverif);
            if($typeid == 3 && $flagverif < 1){
                if(\Hash::check($pass, $password)){
                    //LOGIN OK
                    //MAKE AUTH
                    \Auth::loginUsingId($uid);
    ...
    public function activateAccount($code)
    {
        ...
        if($user){
            ...
            return redirect('confirmation')->with('new', true);
        }
    }
    ...
    public function getVerifikasiMember(Request $Request){
//        $nope = \DB::table('customers')
//            ->Where('member_id', \Auth::User()->id)
//            ->pluck('phone_number');
         $nopo ="";

        return view('auth.verifikasi')->with('nope', $nopo);
    }

    public function cekVerifOTP(Request $Request){

//        if ($Request->ajax()) {
        $otp = $Request->get("pincode");

//            dd($otp);

        $kodemember = OtpMember::where('member_id', \Auth::User()->id)->pluck('kodemember');

        $otpmember = OtpMember::where('kode_otp', $otp)
            ->Where('member_id', \Auth::User()->id)
        ...
        if($otpmember > 0){
            ...
            $UpdateMember = User::where('id', '=',  \Auth::User()->id)->first();
            $UpdateMember->flag_verif = 1;
            $UpdateMember->kodemember = $kodemember;
            $UpdateMember->save();
//                \DB::table('members')
//
//                    ->where('id', \Auth::User()->id)
//                    ->update(['flag_verif' => 1],['kodemember'=>$kodemember]);
...
//        }
    ...
    public function getVerifKodemember(Request $Request){
        $kodemember = $Request->get('kodemember');
        $nohp = $Request->get('nohp');

        $mask_number = substr_replace($nohp, str_repeat("X", 4), 7);

        $countotp = \DB::table('otpmember')
            ->Where('member_id', \Auth::User()->id)
            ->where(\DB::raw('DATE(created_at)'), '=', Carbon::today())
            ->count();


        $countstatus = \DB::table('customers')
            ->Where('kode_member', $kodemember)
            ->Where('phone_number', $nohp)
            ->WhereNull('flag_membermerah')
            ->Where('cus_flagmypoin', 1)
            ->count();


        if($countotp >= 5){
            return redirect('verif')->with('err', 'Anda telah melakukan request OTP sebanyak 5 kali hari ini, Coba kembali setelah 1x24 jam');
        }
        ...
        if($countstatus > 0){
            ...
            $array = [
                'noHP' => $nohp,
                'text' => 'Kode OTP Verifikasi Member Klikindogrosir Anda : ' . $kode_otp . " . Hanya dapat digunakan selama 2 menit."
            ];

            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, '172.31.2.118/smsGateway/public/smsOTP');
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                'X-Authorization: aadd49b7feab69f6c907e93d226cb8de10ae4f6e' 
            ));
            ...
            $reset = new OtpMember();  
            $reset->kodemember = $Request->get('kodemember');
            $reset->handphone = $nohp;
            $reset->member_id = \Auth::User()->id;
            $reset->kode_otp = $kode_otp;
            $reset->status_code = "";
//            $reset->status_code = substr($result, 0, 4);
            $reset->status_text = 'Request has been successfully received';
            $reset->save();


            return redirect('verif')->with('opencart', true)->with('nope', $mask_number);
        }else{
            return redirect('verif')->with('err', 'Kode member atau nomor handphone yang Anda masukkan salah, cobalah periksa kembali');
    ...
    public function ResendOtp(Request $Request){
        ...
        $nohape = \DB::table('otpmember')
            ->Where('member_id', \Auth::User()->id)
            ->Pluck('handphone');

        $kodemember = \DB::table('otpmember')
            ->Where('member_id', \Auth::User()->id)
            ->Pluck('kodemember');
        ...
        if($countotp < 5){
            $array = [
                'noHP' => $nohape,
                'text' => 'Kode OTP Verifikasi Member Klikindogrosir Anda : ' . $kode_otp . " . Hanya dapat digunakan selama 5 menit."
            ];
...
//            curl_close($ch);
            ...
            $reset = new OtpMember();
            $reset->kodemember = $kodemember;
            $reset->handphone = $nohape;
            $reset->member_id = \Auth::User()->id;
            $reset->kode_otp = $kode_otp;
            $reset->status_code = "";
//            $reset->status_code = substr($result, 0, 4);
...
//                \Auth::loginUsingId($newU->id);
    ...
    public function getKonfirmasiView(){
        return view('auth.konfirmasiemail');
    }
